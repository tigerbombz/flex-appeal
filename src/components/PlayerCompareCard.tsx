...
const getMatchupColor = (difficulty: Player['matchupDifficulty']): string => {
...
const getTrendColor = (trend: Player['trend']): string => {
  if (trend === 'up') return '#22c55e';
  if (trend === 'down') return '#ef4444';
  return '#7a8099';
};

const getAdjustedScore = (player: Player, format: 'PPR' | 'Half' | 'Standard'): number => {
...
const PlayerCompareCard = ({ player, onRemove, onToggleLock, scoringFormat, rank }: Props) => {
  const adjustedScore = getAdjustedScore(player, scoringFormat);
  ...
  return (
    ...
    >
            ...
            <Box sx={{ display: 'flex', alignItems: 'center', gap: 0.75 }}>
              ...
              {player.status === 'questionable' && (
                <Chip
                  label="Q"
                  size="small"
                  sx={{ fontSize: 10, height: 18, bgcolor: '#eab30820', color: '#eab308', fontWeight: 700 }}
                />
              )}
              {player.status === 'out' && (
                <Chip
                  label="OUT"
                  size="small"
                  sx={{ fontSize: 10, height: 18, bgcolor: '#ef444420', color: '#ef4444', fontWeight: 700 }}
                />
              )}
            </Box>
      ...
      <Box sx={{ display: 'flex', alignItems: 'center', gap: 1.5 }}>
        <Box
          sx={{
            bgcolor: getScoreColor(adjustedScore),
            borderRadius: 2,
            px: 1.5,
            py: 0.5,
            textAlign: 'center',
        ...
        >
          ...
          <Typography sx={{ fontSize: 9, fontWeight: 700, color: '#000', textTransform: 'uppercase', letterSpacing: 0.5 }}>
            {adjustedScore >= 80 ? 'Start' : adjustedScore >= 65 ? 'Lean' : 'Sit'}
          </Typography>
        </Box>
        ...
        <Box sx={{ display: 'flex', flexDirection: 'column', gap: 0.5 }}>
          ...
          {player.score !== adjustedScore && (
            <Typography sx={{ fontSize: 11, color: 'text.secondary' }}>
              Base: {player.score} → Adjusted: {adjustedScore}
            </Typography>
          )}
        </Box>
      </Box>
      ...
      {player.status === 'questionable' && (
        ...
        >
          ...
          <Typography sx={{ fontSize: 12, color: '#eab308' }}>
            {player.name} is listed as questionable — monitor injury report before locking in.
          </Typography>
        </Box>
      )}
      {player.status === 'out' && (
