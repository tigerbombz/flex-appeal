...
import {
  Box, Typography, Button, CircularProgress, Alert, Chip, TextField, Autocomplete,
} from '@mui/material';
...
const PRIORITY_COLORS: Record<string, string> = {
  high: '#22c55e', medium: '#eab308', low: '#7a8099',
};
...
const WaiverAssistant = () => {
    ...
    const fetchFreeAgents = async () => {
      try {
        ...
        if (data?.players?.length) {
          ...
          setAvailable(data.players.slice(0, 10).map(freeAgentToPlayer));
  ...
  const handleAnalyze = async () => {
    try {
      ...
      const data = await aiApi.analyzeWaiver({
        starters: starters.map((p) => ({
          name: p.name, position: p.position, team: p.team, slot: p.slot,
          score: p.score, volatility: p.volatility, status: p.status,
  ...
  return (
    <Box sx={{ p: 2 }}>
      <Box sx={{ mb: 3 }}>
        <Typography variant="h5" sx={{ fontWeight: 700 }}>Waiver Assistant</Typography>
      ...
      <Box sx={{ bgcolor: 'background.paper', border: '1px solid', borderColor: 'divider', borderRadius: 3, p: 2, mb: 2 }}>
        <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', mb: 1 }}>
          <Typography sx={{ fontSize: 12, fontWeight: 700, color: 'text.secondary', textTransform: 'uppercase', letterSpacing: 1 }}>Your Roster</Typography>
          {rosterLoading ? (
            <Chip label="Loading…" size="small" sx={{ fontSize: 10, height: 18 }} />
          ) : isRealData ? (
        ...
        <Box sx={{ display: 'flex', flexWrap: 'wrap', gap: 0.75 }}>
          {[...starters, ...bench].map((p) => (
            <Chip key={p.id} label={`${p.name} · ${p.position}`} size="small"
              sx={{ fontSize: 11, bgcolor: starters.find((s) => s.id === p.id) ? 'background.default' : '#ffffff10', color: 'text.secondary', border: '1px solid', borderColor: 'divider' }} />
          ))}
        </Box>
      </Box>
      ...
      <Box sx={{ mb: 2 }}>
        <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', mb: 1 }}>
          <Typography sx={{ fontSize: 12, fontWeight: 700, color: 'text.secondary', textTransform: 'uppercase', letterSpacing: 1 }}>Waiver Pool</Typography>
          {freeAgentsLoading ? (
            <Chip label="Fetching waivers…" size="small" sx={{ fontSize: 10, height: 18 }} />
          ) : freeAgentsSource === 'yahoo' ? (
        ...
        {freeAgentsSource === 'yahoo' && freeAgents.length > 0 && (
          <Box sx={{ mb: 1.5 }}>
            <Typography sx={{ fontSize: 11, color: 'text.secondary', mb: 0.75 }}>Top available by ownership %:</Typography>
            <Box sx={{ display: 'flex', flexWrap: 'wrap', gap: 0.5 }}>
              {freeAgents.slice(0, 8).map((fa) => (
                <Chip key={fa.player_key}
                  label={`${fa.name} · ${fa.position} · ${fa.ownership_pct.toFixed(0)}%`}
                  size="small"
                  onClick={() => {
                    const p = freeAgentToPlayer(fa);
                    if (!available.find((a) => a.id === p.id)) setAvailable((prev) => [...prev, p]);
                  }}
                  sx={{
        ...
        <Autocomplete
          multiple value={available} onChange={(_, val) => setAvailable(val)}
          options={autocompleteOptions}
          getOptionLabel={(p) => `${p.name} (${p.position}, ${p.team})`}
          renderValue={(value, getItemProps) => value.map((p, i) => (
            <Chip label={`${p.name} · ${p.position}`} size="small" {...getItemProps({ index: i })} sx={{ fontSize: 11 }} />
          ))}
          renderInput={(params) => (
            <TextField {...params} size="small"
              placeholder={freeAgentsLoading ? 'Loading waiver pool…' : freeAgentsSource === 'yahoo' ? 'Search your waiver wire…' : 'Search players…'}
      ...
      <Button variant="contained" fullWidth size="large"
        ...
        onClick={handleAnalyze} disabled={loading || !available.length}
      ...
      {result && (
        <Box>
          <Box sx={{ bgcolor: 'background.paper', border: '1px solid', borderColor: 'divider', borderRadius: 3, p: 2, mb: 2 }}>
            <Typography sx={{ fontSize: 11, fontWeight: 700, color: 'primary.main', textTransform: 'uppercase', letterSpacing: 1, mb: 0.75 }}>Roster Analysis</Typography>
            <Typography sx={{ fontSize: 13, color: 'text.secondary', lineHeight: 1.6 }}>{result.roster_analysis}</Typography>
            {result.priority_positions?.length > 0 && (
              <Box sx={{ display: 'flex', gap: 0.75, mt: 1.5, flexWrap: 'wrap' }}>
                <Typography sx={{ fontSize: 12, color: 'text.secondary', alignSelf: 'center' }}>Priority positions:</Typography>
                {result.priority_positions.map((pos: string) => (
                  <Chip key={pos} label={pos} size="small" sx={{ fontSize: 11, bgcolor: 'primary.main', color: '#000', fontWeight: 700 }} />
                ))}
              </Box>
            )}
          </Box>

          <Typography sx={{ fontSize: 12, fontWeight: 700, color: 'text.secondary', textTransform: 'uppercase', letterSpacing: 1, mb: 1 }}>Recommendations</Typography>
          <Box sx={{ display: 'flex', flexDirection: 'column', gap: 1.5, mb: 2 }}>
            {result.recommendations?.map((rec: any, i: number) => (
              <Box key={i} sx={{ bgcolor: 'background.paper', border: '1px solid', borderColor: `${PRIORITY_COLORS[rec.priority]}40`, borderRadius: 3, p: 2 }}>
                <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', mb: 1 }}>
                  <Box>
                    <Box sx={{ display: 'flex', alignItems: 'center', gap: 1, mb: 0.25 }}>
                      <Typography sx={{ fontWeight: 700, fontSize: 15 }}>➕ {rec.player}</Typography>
                      <Chip label={rec.position} size="small" sx={{ fontSize: 10, height: 18, bgcolor: 'background.default', color: 'text.secondary' }} />
                    </Box>
                    {rec.drop_player && <Typography sx={{ fontSize: 12, color: '#ef4444' }}>➖ Drop: {rec.drop_player}</Typography>}
                  </Box>
                  <Chip label={rec.priority} size="small" sx={{ fontSize: 11, fontWeight: 700, height: 22, bgcolor: `${PRIORITY_COLORS[rec.priority]}20`, color: PRIORITY_COLORS[rec.priority], border: '1px solid', borderColor: `${PRIORITY_COLORS[rec.priority]}40` }} />
                </Box>
                <Typography sx={{ fontSize: 13, color: 'text.secondary', lineHeight: 1.5 }}>{rec.reasoning}</Typography>
              </Box>
            ))}
          </Box>
          ...
          {result.weekly_tip && (
            <Box sx={{ bgcolor: 'background.paper', border: '1px solid', borderColor: 'primary.main', borderRadius: 3, p: 2 }}>
              <Typography sx={{ fontSize: 11, fontWeight: 700, color: 'primary.main', textTransform: 'uppercase', letterSpacing: 1, mb: 0.75 }}>💡 Weekly Tip</Typography>
              <Typography sx={{ fontSize: 13, lineHeight: 1.6 }}>{result.weekly_tip}</Typography>
            </Box>
          )}
